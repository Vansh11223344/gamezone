...
const Snake = () => {
  ...
  useEffect(() => {
    const handleResize = () => {
      let min = Math.min(window.innerWidth - 26, 430);
  ...
  moveRef.current = () => {
    setSnake(prevSnake => {
      ...
      setDir(currDir); // Update dir to nextDir for next frame
      const nextHead = {
        x: (prevSnake[0].x + DIRECTIONS[currDir].x + BOARD_COLS) % BOARD_COLS,
        y: (prevSnake[0].y + DIRECTIONS[currDir].y + BOARD_ROWS) % BOARD_ROWS,
      };
  ...
  // Speed can only be changed when stopped or game over
  const canChangeSpeed = (!playing && !gamePaused) || gameOver;

  const scale = containerWidth / (BOARD_COLS * SQUARE_SIZE);

  return (
    <div className="snake-nokia-bg">
      <div className="snake-nokia-container">
        ...
        <div
          ...
          style={{
            width: BOARD_COLS * SQUARE_SIZE * scale,
            height: BOARD_ROWS * SQUARE_SIZE * scale,
